...
class Codegen {
  ...
  async generate(done) {
    ...
    define('module.c', `// auto generated: ${Date()}
      #include <stdio.h>
      #include <string.h>
      #include <sodium.h>
      #include <node_api.h>
  ...
  "targets": [{
    ...
    "conditions": [
      ["OS == 'linux'", {
        "link_settings": {
          "libraries": [
            "<!(node -p \\"require('hidden-machine/ldflags')\\")"
          ]
        }
      }]
    ]
  }, {
    "target_name": "module_mac",
    "dependencies": [ "module_build" ],
    "type": "none",
    "conditions": [
      ["OS == 'mac'", {
        "actions": [{
          ...
          "inputs": [],
          "outputs": [ "./build/Release/module_build.node" ],
          "action": [
            "install_name_tool",
            "-change",
            "@loader_path/libsodium.dylib",
            "<!(node -p \\"require('hidden-machine/libraries/sodium')\\")",
            "./build/Release/module_build.node"
          ]
        }]
      }]
    ]
    ...
    "target_name": "module",
    "type": "none",
    "dependencies": [ "module_build", "module_mac" ],
    "actions": [{
      "action_name": "rename_module_build",
      "inputs": [ ],
      "outputs": [ "./build/Release/module.node" ],
      "action": [
        "mv",
        "-f",
        "./build/Release/module_build.node",
    ...
    function define(filename, source) {
      assets[filename] = {
        source: Buffer.from(strip(source.trim()))
      }
    }
  }
}
