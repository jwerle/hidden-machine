...
{ # auto generated: ${Date()}
  "targets": [{
    "target_name": "module",
    "sources": [ "module.c" ],
    "variables": { "target_arch%": "<!(node -p \\"os.arch()\\")>" },

    "cflags": [ "-O3", "-std=c99", "-D_GNU_SOURCE" ],
    "libraries": [ "<!(node -e \\"require('libsodium-prebuilt/preinstall')\\"  -- --print-lib)" ],
    ...
    "conditions": [
      ...
      ["OS == 'win'", {
        "link_settings": {
          "libraries": [ "<!(node -p \\"require('libsodium-prebuilt/paths').win32lib\\")" ]
        }
      }],
